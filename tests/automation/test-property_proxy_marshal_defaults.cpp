...
struct visitor {
    ...
    void operator()(refl::property_proxy<Ty_, false> pr) {
        auto stringfy_trivial = [this](auto const& pv) {
            ...
            if constexpr (marshal::generic_is_trivially_marshalable_v<type>) {
                std::string ostr1, ostr2;
                marshal::generic_stringfy<type>{}(pv, std::back_inserter(ostr1));

                type v;
                marshal::generic_parse<type>{}(ostr1.begin(), ostr1.end(), v);
                marshal::generic_stringfy<type>{}(v, std::back_inserter(ostr2));
